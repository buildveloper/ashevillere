...
## Build, lint, test
- Run `pnpm build` before considering any task finished.
- Run `pnpm lint` and fix errors (warnings are a judgment call).
- If TypeScript is configured, run the typecheck script too.
- Run `pnpm test` — 22 tests currently, all must stay green. If a change touches parcel resolution, geocoding, or any rule-based logic (flood, STR, recovery), add or update tests in the same commit, not after.
...
- Never remove, disable, or architecturally replace a feature outside what the current prompt explicitly scopes — including something that appears broken or not worth the cost. If something seems like it should be cut or changed beyond scope, stop, report exactly what you found and why, and propose the change. Wait for it to be confirmed before acting on it. This applies even under `--yolo`/background mode — autonomy on execution within scope is not the same as authority to change scope.
